		...
		s_arr_employees *employees_array)
{
	...
	for (i = 0; i < spreadsheet->used; i++)
	{
		...
		spreadsheet->details[i].food_allowance =
				...
				(float) spreadsheet->details[i].weekend_days *
				employees_array->employees[emp_index].base_food_allowance;
		printf("Food: %.2f€\n", spreadsheet->details[i].food_allowance);
		...
		{
			case NONE:
				spreadsheet->details[i].irs_retention = h_proc_get_retention_percentage(single_array,
																						employees_array->employees[emp_index].dependents,
																						spreadsheet->details[i].gross_pay);
				break;
			case UNIQUE_HOLDER:
				spreadsheet->details[i].irs_retention = h_proc_get_retention_percentage(unique_holder_array,
																						employees_array->employees[emp_index].dependents,
																						spreadsheet->details[i].gross_pay);
				break;
			case TWO_HOLDERS:
				spreadsheet->details[i].irs_retention = h_proc_get_retention_percentage(two_holders_array,
																						employees_array->employees[emp_index].dependents,
																						spreadsheet->details[i].gross_pay);
				break;
		}
